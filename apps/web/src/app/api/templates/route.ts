...
function logProxyError(error: unknown): void {
  ...
  console.error("api/templates proxy failed", { message });
}
...
export async function GET() {
  ...
  try {
    const upstreamResponse = await fetch(`${apiBaseUrl}/api/v1/templates`, {
      method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/templates proxy failed", {
