...
function upstreamHeaders(accessToken: string | null): HeadersInit {
  if (!accessToken) {
    return {
...
function logProxyError(error: unknown): void {
  ...
  console.error("api/templates/[slug] proxy failed", { message });
}
...
export async function GET(_request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/templates/${encodeURIComponent(normalizedSlug)}`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/templates/[slug] proxy failed", {
