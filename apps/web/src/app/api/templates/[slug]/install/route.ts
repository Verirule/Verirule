...
function logProxyError(error: unknown): void {
  ...
  console.error("api/templates/[slug]/install proxy failed", { message });
}
...
export async function POST(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/templates/${encodeURIComponent(normalizedSlug)}/install`,
      {
        method: "POST",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/templates/[slug]/install proxy failed", {
