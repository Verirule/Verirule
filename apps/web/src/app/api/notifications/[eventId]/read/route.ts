...
async function proxyReadState(
  ...
  if (!cleanedEventId) {
    return apiError(400, "eventId is required", requestId);
  }

  try {
    const upstream = await fetchWithTimeout(
      `${apiBaseUrl}/api/v1/notifications/${encodeURIComponent(cleanedEventId)}/read`,
      {
        method,
        headers: {
    ...
    console.error("notification read-state proxy failed", { request_id: requestId });
...
export async function POST(
  ...
  return proxyReadState(request, eventId, "POST");
}
...
export async function DELETE(
  ...
  const { eventId } = await context.params;
  return proxyReadState(request, eventId, "DELETE");
}
