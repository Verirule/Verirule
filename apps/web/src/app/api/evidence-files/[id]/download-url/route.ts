...
export async function GET(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/evidence-files/${encodeURIComponent(evidenceFileId)}/download-url?org_id=${encodeURIComponent(orgId)}`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/evidence-files/[id]/download-url proxy failed", {
    ...
    console.error("api/evidence-files/[id]/download-url proxy failed", { message });
    return NextResponse.json({ message: "Upstream API error" }, { status: 502 });
  }
}
