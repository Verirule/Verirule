...
export async function POST(
  ...
  if (!evidenceFileId) {
    return NextResponse.json({ message: "Invalid evidence file id" }, { status: 400 });
  }
  ...
  if (!orgId || !sha256) {
    return NextResponse.json({ message: "Invalid finalize payload" }, { status: 400 });
  }

  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/evidence-files/${encodeURIComponent(evidenceFileId)}/finalize`,
      {
        method: "POST",
        headers: upstreamHeaders(accessToken),
        body: JSON.stringify({ org_id: orgId, sha256 }),
        cache: "no-store",
      },
    );
    if (!upstreamResponse.ok) {
      console.error("api/evidence-files/[id]/finalize proxy failed", {
    ...
    console.error("api/evidence-files/[id]/finalize proxy failed", { message });
    return NextResponse.json({ message: "Upstream API error" }, { status: 502 });
  }
}
