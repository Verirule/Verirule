...
function logProxyError(error: unknown): void {
  ...
  console.error("api/integrations/jira/create-issue proxy failed", { message });
}
...
export async function POST(request: NextRequest) {
  ...
  const payload = (await request.json().catch(() => null)) as
    | { org_id?: unknown; alert_id?: unknown }
    | null;
  ...
  if (!orgId || !alertId) {
    return NextResponse.json({ message: "Invalid Jira issue payload" }, { status: 400 });
  }

  try {
    const upstreamResponse = await fetch(`${apiBaseUrl}/api/v1/integrations/jira/create-issue`, {
      method: "POST",
      headers: upstreamHeaders(accessToken),
      body: JSON.stringify({ org_id: orgId, alert_id: alertId }),
      cache: "no-store",
    });

    if (!upstreamResponse.ok) {
      console.error("api/integrations/jira/create-issue proxy failed", {
