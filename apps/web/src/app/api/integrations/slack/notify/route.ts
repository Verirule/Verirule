...
function logProxyError(error: unknown): void {
  ...
  console.error("api/integrations/slack/notify proxy failed", { message });
}
...
export async function POST(request: NextRequest) {
  ...
  if (!orgId || !alertId) {
    return NextResponse.json({ message: "Invalid Slack notify payload" }, { status: 400 });
  }

  try {
    const upstreamResponse = await fetch(`${apiBaseUrl}/api/v1/integrations/slack/notify`, {
      method: "POST",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/integrations/slack/notify proxy failed", {
