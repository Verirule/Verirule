...
function logProxyError(error: unknown): void {
  ...
  console.error("api/controls proxy failed", { message });
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstreamResponse = await fetch(`${apiBaseUrl}/api/v1/controls${query}`, {
      method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/controls proxy failed", {
