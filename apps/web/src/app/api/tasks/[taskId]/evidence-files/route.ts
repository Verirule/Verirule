...
export async function GET(
  ...
  if (!trimmedTaskId || !orgId) {
    return NextResponse.json({ message: "taskId and org_id are required" }, { status: 400 });
  }

  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/tasks/${encodeURIComponent(trimmedTaskId)}/evidence-files?org_id=${encodeURIComponent(orgId)}`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/tasks/[taskId]/evidence-files proxy failed", {
    ...
    console.error("api/tasks/[taskId]/evidence-files proxy failed", { message });
    return NextResponse.json({ message: "Upstream API error" }, { status: 502 });
  }
}
