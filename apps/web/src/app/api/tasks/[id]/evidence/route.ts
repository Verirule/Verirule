...
function logProxyError(error: unknown): void {
  ...
  console.error("api/tasks/[id]/evidence proxy failed", { message });
}
...
export async function GET(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/tasks/${encodeURIComponent(taskId)}/evidence`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/tasks/[id]/evidence proxy failed", {
...
export async function POST(
  ...
  const payload = (await request.json().catch(() => null)) as { type?: unknown; ref?: unknown } | null;
  const type =
    payload?.type === "link" || payload?.type === "file" || payload?.type === "log"
      ? payload.type
      : null;
  const ref = typeof payload?.ref === "string" ? payload.ref.trim() : "";
  if (!type || !ref) {
    return NextResponse.json({ message: "Invalid evidence payload" }, { status: 400 });
  }
