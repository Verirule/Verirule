...
function logProxyError(error: unknown): void {
  ...
  console.error("api/tasks/[id]/evidence/[evidenceId]/download-url proxy failed", { message });
}
...
export async function GET(
  ...
  const taskEvidenceId = evidenceId?.trim() ?? "";
  if (!taskId || !taskEvidenceId) {
    return NextResponse.json({ message: "Invalid task evidence id" }, { status: 400 });
  }

  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/tasks/${encodeURIComponent(taskId)}/evidence/${encodeURIComponent(taskEvidenceId)}/download-url`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/tasks/[id]/evidence/[evidenceId]/download-url proxy failed", {
