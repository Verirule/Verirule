...
function logProxyError(error: unknown): void {
  ...
  console.error("api/tasks/[id]/evidence/file proxy failed", { message });
}
...
export async function POST(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/tasks/${encodeURIComponent(taskId)}/evidence/file`,
      {
        method: "POST",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/tasks/[id]/evidence/file proxy failed", {
