...
export async function GET(
  ...
  if (limit && limit.trim()) {
    upstreamParams.set("limit", limit.trim());
  }
  if (status && status.trim()) {
    upstreamParams.set("status", status.trim());
  }
  if (userId && userId.trim()) {
    upstreamParams.set("user_id", userId.trim());
  }

  const upstreamUrl = `${apiBaseUrl}/api/v1/orgs/${encodeURIComponent(cleanedOrgId)}/notifications/inbox`;
  const targetUrl = upstreamParams.size > 0 ? `${upstreamUrl}?${upstreamParams.toString()}` : upstreamUrl;
  ...
  try {
    ...
    console.error("notifications inbox proxy failed", { request_id: requestId });
