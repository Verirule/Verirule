...
function logProxyError(error: unknown): void {
  ...
  console.error("api/orgs/[orgId]/controls proxy failed", { message });
}
...
export async function GET(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/orgs/${encodeURIComponent(normalizedOrgId)}/controls`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/orgs/[orgId]/controls proxy failed", {
