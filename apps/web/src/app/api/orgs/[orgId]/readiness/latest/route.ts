...
export async function GET(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/orgs/${encodeURIComponent(normalizedOrgId)}/readiness/latest`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/orgs/[orgId]/readiness/latest proxy failed", {
    ...
    console.error("api/orgs/[orgId]/readiness/latest proxy failed", { message });
    return NextResponse.json({ message: "Upstream API error" }, { status: 502 });
  }
}
