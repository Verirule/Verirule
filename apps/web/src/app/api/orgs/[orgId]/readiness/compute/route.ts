...
export async function POST(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/orgs/${encodeURIComponent(normalizedOrgId)}/readiness/compute`,
      {
        method: "POST",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/orgs/[orgId]/readiness/compute proxy failed", {
    ...
    console.error("api/orgs/[orgId]/readiness/compute proxy failed", { message });
    return NextResponse.json({ message: "Upstream API error" }, { status: 502 });
  }
}
