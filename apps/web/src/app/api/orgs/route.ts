...
function logProxyError(error: unknown): void {
  ...
  console.error("api/orgs proxy failed", { message });
}
...
export async function GET() {
  ...
  try {
    const upstreamResponse = await fetch(`${apiBaseUrl}/api/v1/orgs`, {
      method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/orgs proxy failed", {
...
export async function POST(request: NextRequest) {
  ...
  if (!name.trim()) {
    return NextResponse.json({ message: "Invalid org payload" }, { status: 400 });
  }
