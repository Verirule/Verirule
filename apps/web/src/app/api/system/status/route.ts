...
function logProxyError(error: unknown): void {
  ...
  console.error("api/system/status proxy failed", { message });
}
...
export async function GET() {
  ...
  try {
    const upstreamResponse = await fetch(`${apiBaseUrl}/api/v1/system/status`, {
      method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/system/status proxy failed", {
