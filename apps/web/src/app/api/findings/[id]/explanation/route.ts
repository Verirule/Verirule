...
function logProxyError(error: unknown): void {
  ...
  console.error("api/findings/[id]/explanation proxy failed", { message });
}
...
export async function GET(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/findings/${encodeURIComponent(findingId)}/explanation`,
      {
        method: "GET",
    ...
    if (upstreamResponse.status === 404) {
      return NextResponse.json({ message: "Explanation not found" }, { status: 404 });
    }

    if (!upstreamResponse.ok) {
      console.error("api/findings/[id]/explanation proxy failed", {
