...
function logProxyError(error: unknown): void {
  ...
  console.error("api/findings/[id]/controls/suggest proxy failed", { message });
}
...
export async function GET(
  ...
  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/findings/${encodeURIComponent(findingId)}/controls/suggest?org_id=${encodeURIComponent(orgId)}`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/findings/[id]/controls/suggest proxy failed", {
