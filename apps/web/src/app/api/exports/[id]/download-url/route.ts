...
function logProxyError(error: unknown): void {
  ...
  console.error("api/exports/[id]/download-url proxy failed", { message });
}
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!exportId) {
    return NextResponse.json({ message: "Invalid export id" }, { status: 400 });
  }

  try {
    const upstreamResponse = await fetch(
      `${apiBaseUrl}/api/v1/exports/${encodeURIComponent(exportId)}/download-url`,
      {
        method: "GET",
    ...
    if (!upstreamResponse.ok) {
      console.error("api/exports/[id]/download-url proxy failed", {
