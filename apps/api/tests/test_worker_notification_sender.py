...
def test_notification_sender_handles_sla_job(monkeypatch) -> None:
    running_calls: list[tuple[str, int]] = []
    sent_calls: list[tuple[str, int]] = []
    sent_messages: list[dict[str, str]] = []
    event_rows: list[dict[str, object]] = []
        ...
        return [
            {
                ...
                "payload": {
                    ...
                    "kind": "overdue",
                    "task_title": "Rotate API keys",
                    "severity": "high",
                    "due_at": "2026-02-15T08:00:00Z",
                    "channel": "email",
                },
                "attempts": 0,
            }
    ...
    async def fake_mark_failed(*args, **kwargs) -> None:
        raise AssertionError("SLA job should not fail in this test")

    async def fake_member_emails(org_id: str):
        assert org_id == ORG_ID
        return [{"user_id": "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", "user_email": "owner@example.com"}]

    async def fake_prefs(user_ids: list[str]):
        assert user_ids == ["aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"]
        return {"aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa": True}

    async def fake_org_name(org_id: str) -> str:
        return "Acme"

    def fake_send_email(*, to: str, subject: str, html: str, text: str, request_id: str | None = None) -> None:
        sent_messages.append({"to": to, "subject": subject, "html": html, "text": text})
    ...
    monkeypatch.setattr(notification_sender, "mark_notification_job_sent", fake_mark_sent)
    monkeypatch.setattr(notification_sender, "mark_notification_job_failed", fake_mark_failed)
    monkeypatch.setattr(notification_sender, "list_org_member_emails", fake_member_emails)
    monkeypatch.setattr(notification_sender, "select_user_notification_prefs_for_users_service", fake_prefs)
    monkeypatch.setattr(notification_sender, "select_org_name_service", fake_org_name)
    monkeypatch.setattr(notification_sender, "send_email", fake_send_email)
    monkeypatch.setattr(notification_sender, "run_in_threadpool", fake_run_in_threadpool)
    monkeypatch.setattr(notification_sender, "rpc_record_audit_event", fake_audit)
    monkeypatch.setattr(notification_sender, "upsert_notification_event_service", fake_upsert_event)
    ...
    assert processed == 1
    assert running_calls == [("job-3", 1)]
    assert sent_calls == [("job-3", 1)]
    assert len(sent_messages) == 1
    assert sent_messages[0]["to"] == "owner@example.com"
    assert sent_messages[0]["subject"].startswith("Overdue: Remediation task past due - ")
    assert [row["event_type"] for row in event_rows] == ["sla", "sla"]
